...
//u8 RealySendMgr[BeaconRelayMax];
//u8 BeaconRelayCnt=0;
//u8 RelaySendCnt=0;
//u8 GetMatchBeacon=0;
...
int main(void)
{
    ...
    Watch_15 *Wptr15=(Watch_15 *)Buffer;
        ...
        if(TickCounter % 100 == 0)
        {
            ...
            {		    
                ...
                {                              
                    ...
                    for(tpye=0;tpye<3;tpye++)
                    {                       
                        for(u8 j=0;j<RMGR[tpye].BeaconRelayCnt;j++)
                        {
                           if(RMGR[tpye].RealySendMgr[j]==1)
                           { 
                                RelayFlag=1; 
                                RelaySend(tpye,j);  
                                RMGR[tpye].RelaySendCnt=j;
                                break;
                           } 
...
/*---------------------------------------------------------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------------------------------------------------------*/  
                    if(Wptr15->Header==0x55AA && Wptr15->SensorType==0x60)//15手表__REV
                    {
                        RX_cnt++;	
                        
                        BeaconRelayStorage(Wptr15,WATCH_15);  
                        seqno0++;                  
                                                              
                    }				
                    else if(Wptr->Header==0xAA && Wptr->Tail==0xAA)//网安手表
                    {
                        RX_cnt++;	
                      
                        LED_R_Blink;
                        BeaconRelayStorage(Wptr,WATCH_WA);  
                      
                        seqno1++;                      
                                                                
                    }
                    else if(Gpstr->head.Preamble==0xFF)                  
                    {                  
                    
                        cc=CalcSum((u8*)&(Gpstr->head.len),Gpstr->head.len-1); 
                    
                        switch(Gpstr->head.cmd)
                        {                                               
                            case Upload_Coordinate:
                            {                                                      
                               if(Gpstr->tail==0xF0 && cc==Gpstr->cc)                                                                      
                               {
                              
                                   LED_G_Blink;
                                   BeaconRelayStorage(Gpstr,BEACON);  
                ...
                {                    
                    ...
                    {                                                      
                       // LED_B_Blink; 
                        ...
                        RMGR[tpye].RelaySendCnt++; 
                        
Relaying:                  
                        while(RMGR[tpye].RelaySendCnt < RMGR[tpye].BeaconRelayCnt)
                        {
                            if(RMGR[tpye].RealySendMgr[RMGR[tpye].RelaySendCnt]==1)                                                     
                            {                                                             
                                bsp_DelayMS(1000); 
                                RelaySend(tpye,RMGR[tpye].RelaySendCnt); 
                        ...
                        if(RMGR[tpye].RelaySendCnt == RMGR[tpye].BeaconRelayCnt)
                        {
                            tpye++;                                                                                             
                            if(tpye>=3)
                            {
                             ...
                             // LED_B_OFF;
                              ...
                              for(u8 i=0;i<3;i++)
                              {
                                  memset((u8*)&RMGR[i].RealySendMgr,0,BeaconRelayMax);
                                  RMGR[i].RelaySendCnt=0;
                              }
